...
const AdminDashboard = () => {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-heading font-bold text-gray-800">{t('Admin Overview')}</h1>
        <p className="text-gray-500 text-sm mt-1">{t("Welcome back. Here's what's happening today.")}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-gray-500 font-medium">{t('Total Revenue')}</p>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-gray-500 font-medium">{t('Total Bookings')}</p>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-gray-500 font-medium">{t('Total Users')}</p>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-gray-500 font-medium">{t('Trek Packages')}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-100 flex justify-between items-center">
            ...
            <Link to="/admin/bookings" className="text-sm text-accent-500 hover:text-accent-600 font-medium">{t('View All')}</Link>
          </div>
          {stats?.recentBookings?.length > 0 ? (
              ...
              {stats.recentBookings.map(b => (
                <div key={b._id} className="p-4 sm:p-6 hover:bg-gray-50 transition-colors flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1 line-clamp-1">{t('Booked')}: {b.trek?.title}</p>
                  </div>
            ...
            <div className="p-6 text-center text-sm text-gray-500">{t('No recent bookings')}</div>
          )}
        </div>

        {/* Quick Actions (Phase placeholder UI) */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
           <h3 className="font-heading font-bold text-gray-800 mb-6">{t('Quick Actions')}</h3>
           <div className="grid grid-cols-2 gap-4">
              <Link to="/admin/treks/add" className="p-4 border border-gray-100 hover:border-primary-500 rounded-xl flex flex-col items-center justify-center gap-3 text-center transition-colors group">
                ...
                <span className="text-sm font-medium text-gray-700 group-hover:text-primary-500">{t('Create Trek')}</span>
              </Link>
              <Link to="/admin/bookings" className="p-4 border border-gray-100 hover:border-accent-500 rounded-xl flex flex-col items-center justify-center gap-3 text-center transition-colors group">
                ...
                <span className="text-sm font-medium text-gray-700 group-hover:text-accent-500">{t('View Bookings')}</span>
              </Link>
           </div>
        </div>
